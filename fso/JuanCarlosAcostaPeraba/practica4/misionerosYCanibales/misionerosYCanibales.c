...
pthread_cond_t misioneros = PTHREAD_COND_INITIALIZER;
pthread_cond_t canibales = PTHREAD_COND_INITIALIZER;

// mutex para controlar el acceso a la variable de estado
...
void *Misionero(void *item)
{
    ...
    {
        pthread_cond_wait(&misioneros, &mutex);
    }
    ...
    if (numMisionerosEnTierra + numCanibalesEnTierra <= 3)
    {
        ...
        if (numMisionerosEnTierra == 2 && numCanibalesEnTierra == 1)
        {
            ...
            // llamada a los hilos de los canibales y misioneros que están esperando
            pthread_cond_broadcast(&canibales);
            pthread_cond_broadcast(&misioneros);
    ...
    if (flag == 1)
    {
        ...
        // se llama a los hilos de los canibales y misioneros que están esperando
        pthread_cond_broadcast(&canibales);
        pthread_cond_broadcast(&misioneros);
...
void *Canibal(void *item)
{
    ...
    {
        // el hilo espera
        pthread_cond_wait(&canibales, &mutex);
    }
    // en tierra quedan 3 o menos personas
    if (numMisionerosEnTierra + numCanibalesEnTierra <= 3)
    {
